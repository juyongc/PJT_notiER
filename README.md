...
- **해결방안**
  ...
  ​ 성능 최적화를 위해 현재 발급 가능한 쿠폰 수를 Redis에서 처리하도록 만들었다.

  ​ 동시성 제어도 쿠폰 수량만 제어하면 되기에 원자적 연산으로 제어했다.

  ​ 이 과정에서 Redis는 MySQL처럼 자동으로 트랜잭션에서 예외가 발생하면 롤백되지 않는다는 걸 알게 되었다.

  ​ 따라서, 예외가 발생하면 이전 데이터로 롤백하도록 직접 로직을 추가해서 데이터 일관성을 유지할 수 있도록 만들었다.
